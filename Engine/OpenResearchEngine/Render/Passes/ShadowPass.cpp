...
ShadowPass::ShadowPass(
    ...
    ShadowResources* shadowResources, RenderTextures* renderTextures)
    : mRootSig(rootSig), mPso(pso),
      mShadowResources(shadowResources), mRenderTextures(renderTextures)
{}
...
void ShadowPass::Execute(const RenderContext& ctx, FrameResource* fr)
{
    ...
    const auto& lights = ctx.sceneState->lights;
    for (int i = 0; i < (int)lights.LightTransforms.size(); i++)
    {
        ...
        ctx.cmdList->ClearDepthStencilView(
            mShadowResources->shadowMaps[i]->Dsv(),
            D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, nullptr);
        ctx.cmdList->OMSetRenderTargets(0, nullptr, false,
            &mShadowResources->shadowMaps[i]->Dsv());
