...
MeshAnimationResource::MeshAnimationResource(Microsoft::WRL::ComPtr<ID3D12Device>& md3dDevice, Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& mCommandList, MeshDeformationData& deformationData)
{
	...
	const UINT simMeshTensionBufferByteSize = (UINT)deformationData.simMeshVertices.size() * sizeof(Vector4);
	ThrowIfFailed(D3DCreateBlob(simMeshTensionBufferByteSize, &SimMeshTensionBufferCPU));
	CopyMemory(SimMeshTensionBufferCPU->GetBufferPointer(), deformationData.simMeshTension.data(), simMeshTensionBufferByteSize);
	SimMeshTensionBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(), mCommandList.Get(), deformationData.simMeshTension.data(), simMeshTensionBufferByteSize, SimMeshTensionBufferUploader);
	mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(SimMeshTensionBufferGPU.Get(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE));
