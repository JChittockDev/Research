...
{
	DirectX::XMMATRIX meshToBoneTransform =
		DirectX::XMMATRIX(aiMatrix.a1, aiMatrix.a2, aiMatrix.a3, aiMatrix.a4,
...
void aiMatConvertTranspose(const aiMatrix4x4& aiMatrix, DirectX::XMFLOAT4X4& dXMatrix)
{
	DirectX::XMMATRIX meshToBoneTransform = XMMatrixTranspose(
		DirectX::XMMATRIX(aiMatrix.a1, aiMatrix.a2, aiMatrix.a3, aiMatrix.a4,
			aiMatrix.b1, aiMatrix.b2, aiMatrix.b3, aiMatrix.b4,
			aiMatrix.c1, aiMatrix.c2, aiMatrix.c3, aiMatrix.c4,
			aiMatrix.d1, aiMatrix.d2, aiMatrix.d3, aiMatrix.d4));
...
void Skeleton::GetTransforms(float timePos, aiNode* node, aiAnimation* animation, aiMatrix4x4& parentTransform, const aiMatrix4x4& globalInverseTransform, std::vector<DirectX::XMFLOAT4X4>& transforms)
{
    ...
    if (pNodeAnim)
    {
        ...
        CalcInterpolatedScaling(scaling, timePos, pNodeAnim);
        CalcInterpolatedRotation(rotation, timePos, pNodeAnim);
        CalcInterpolatedPosition(translate, timePos, pNodeAnim);
