...
void EngineApp::SetBendingConstraintSolveRootSignature()
{
    // Define root signature parameters
    CD3DX12_ROOT_PARAMETER rootParameters[5];
    ...
    ThrowIfFailed(hr);
    ThrowIfFailed(md3dDevice->CreateRootSignature(0, serializedRootSig->GetBufferPointer(), serializedRootSig->GetBufferSize(), IID_PPV_ARGS(mBendingConstraintSolveRootSignature.GetAddressOf())));
}
