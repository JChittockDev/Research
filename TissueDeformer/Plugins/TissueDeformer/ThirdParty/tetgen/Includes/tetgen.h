...
#define fastlookup(pool, index) \
	...
	static void facenormal( point pa, point pb, point pc, REAL *n, int pivot, REAL *lav );
	...
	void projpt2edge( REAL* p, REAL* e1, REAL* e2, REAL* prj, REAL* uv = NULL );
	...
	enum interresult scoutsegment( point, point, face*, triface*, point*,
								   arraypool* );
	int  getsteinerptonsegment( face* seg, point refpt, point steinpt, REAL* uv = NULL );
	...
	int checkfac4encroach( point, point, point, point checkpt, REAL*, REAL*, REAL* uv = NULL );
	int checkfac4split( face *chkfac, point& encpt, int& qflag, REAL *ccent, REAL* uv = NULL );
	int splitsubface( face *splitfac, point, point, int qflag, REAL *ccent, REAL* uv, int );
	...
	void initializetetgenmesh( )
	{
		...
		samples = 0l;
		randomseed = 1l;
		minfaceang = minfacetdihed = 3.14159265359;
		tetprism_vol_sum = 0.0;
